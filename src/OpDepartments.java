		...
		try {
			...
			if(rs.next()) {
				if(rs.getDate("time_to").compareTo(now) > 0) {
					if(i == 0) {
						String subquery = "update Departments_Managers set time_to=TO_DATE('" + now + "', 'YYYY-MM-DD HH24:MI:SS') where ssn='" + ssn + "' and did=" + did;
					...
					if(i == 1) {
						...
						if(rs2.next()) {
							if(rs2.getDate("time_to").compareTo(now) > 0) {
								...
								subquery = "update Departments_Managers set time_to=TO_DATE('" + now + "', 'YYYY-MM-DD HH24:MI:SS') where ssn='" + ssnnow + "' and did=" + did;
								st.executeUpdate(subquery);
							}
						}
						subquery = "insert into Departments_Managers (time_from, time_to, did, ssn) values ( TO_DATE('" + now + "', 'YYYY-MM-DD HH24:MI:SS'), TO_DATE('2099-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'), " + did + ", '" + ssn + "')";
						st.executeUpdate(subquery);
						rs2.close();
					}
				}
				...
				if(i == 1) {
					String subquery = "insert into Departments_Managers (time_from, time_to, did, ssn) values ( TO_DATE('" + now + "', 'YYYY-MM-DD HH24:MI:SS'), TO_DATE('2099-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'), " + did + ", '" + ssn + "')";
					st.executeUpdate(subquery);
				}
			}
			rs.close();
	...
	public void uptDeptManager(String ssn, int did) {
		...
		try {
			...
			if(rs2.next()) {
				if(rs2.getDate("time_to").compareTo(now) > 0) {
					String ssnnow = rs2.getString("ssn");
					subquery = "update Departments_Managers set time_to=TO_DATE('" + strnow + "', 'YYYY-MM-DD HH24:MI:SS') where ssn='" + ssnnow + "' and did=" + did;
					st.executeUpdate(subquery);
				}
			}
			rs2.close();
...
//				}
//			}
			subquery = "insert into Departments_Managers (time_from, time_to, did, ssn) values ( TO_DATE('" + strnow + "', 'YYYY-MM-DD HH24:MI:SS'), TO_DATE('2099-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'), " + did + ", '" + ssn + "')";
			st.executeUpdate(subquery);
//			rs2.close();
//			st2.close();
		    st.close();
